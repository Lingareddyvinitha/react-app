import styled from '@emotion/styled';
import tw from 'tailwind.macro';

const Container = styled.div `${tw `flex justify-center items-center flex-col w-full m-2` }`

const Group = styled.div `${tw `flex justify-start items-start w-full` }`

const Image = styled.img `${tw `w-20 h-20`}`

const Info = styled.div `${tw `flex justify-start items-start flex-col m-2`}`

const Amount = styled.div `${tw `text-yellow-500`}`

const Title = styled.div `${tw `text-white`}`

const Style = styled.div `${tw `text-gray-400`}`

const Quantity = styled.div `${tw `text-gray-400`}`
export {
    Container,
    Group,
    Image,
    Info,
    Amount,
    Title,
    Style,
    Quantity
}
